...
/* w_block_evaluate_host.cu */
unsigned int * w_block_precomputed(unsigned char * salt);

/* password_host.cu */
int readFilePassword(char *buf, int maxNumPsw, FILE *fp);
void readData(FILE *fp, unsigned char salt[SALT_SIZE], unsigned char mac[MAC_SIZE], unsigned char nonce[NONCE_SIZE], unsigned char encryptedVMK[VMK_SIZE]);

/* dictionary_host.cu */
char *crack_dict(char *dname, uint32_t * w_blocks_d, unsigned char * encryptedVMK, unsigned char * nonce,  int gridBlocks);
void setBufferPasswordSize(size_t avail, size_t * passwordBufferSize, int * numPassword);

/* utils.cu */
int getNumberOfThreads(int password_count, int maxThreadsPerBlock, int multiProcessorCount);
void * Calloc(size_t len, size_t size);
void cpu_print_hex(unsigned char hash[], int size);
#endif
